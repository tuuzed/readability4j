...
public class ReadabilityCoreExtractor implements ReadabilityExtractor {
    ...
    public Element fetchArticleContent(Document document) {
        ...
        List<Element> nodesToScore = nodePrepping(allElementsList, document);

        List<Element> candidates = calculateContentScore(nodesToScore);
        ...
        if (siblingNodes.size() > 0) {
            for (Element siblingNode : siblingNodes) {
                ...
                log.debug("Looking at sibling node: [" + siblingNode.getClass() + "] (" + siblingNode.className()
                        + ":" + siblingNode.id() + ")");
                if (scored) {
                    log.debug("Sibling has score " + ScoreInfo.getContentScore(siblingNode));
                } else {
