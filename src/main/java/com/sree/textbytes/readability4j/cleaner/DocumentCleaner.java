...
public class DocumentCleaner {
    ...
    public Document clean(Document doc) {
        log.debug("Starting cleaning phase with DocumentCleaner : " + doc);
        this.docToClean = doc;
    ...
    private void removeStyleSheets(Document docToClean) {
        ...
        log.debug("Removing " + stylesheets.size() + " style sheets");
        stylesheets.remove();
    }
    ...
    private void convertNoScriptToDiv(Document docToClean) {
        ...
        for (Element noScript : noScripts) {
            log.debug("Converting NO SCRIPT to DIV");
    ...
    private void cleanComments(Node node) {
        ...
        while (i < node.childNodes().size()) {
            ...
            if (child.nodeName().equals("#comment")) {
                log.debug("Cleaning comment tag " + child);
                child.remove();
            } else {
                cleanComments(child);
                i++;
    ...
    private void removeEmptyParas(Document docToClean) {
        ...
        for (Element para : paras) {
            if (string.isNullOrEmpty(para.text()) && para.childNodes().size() == 0) {
                log.debug("Null Para found :" + para + "size : " + para.childNodes().size());
                para.remove();
            }
        }
    }
    ...
    private void removeDropCaps(Document doc) {
        ...
        log.debug("Cleaning " + items.size() + " dropcap tags");
    ...
    private void cleanBadTags(Document doc) {
        ...
        log.debug(naughtyList.size() + " naughty ID elements found");
        for (Element node : naughtyList) {
            log.debug("Cleaning: Removing node with id: " + node.id());
            removeNode(node);
        }
        Elements naughtyList2 = children.select(queryNaughtyIDs);
        log.debug(naughtyList2.size()
                + " naughty ID elements found after removal");

        Elements naughtyList3 = children.select(queryNaughtyClasses);
        log.debug(naughtyList3.size() + " naughty CLASS elements found");
        for (Element node : naughtyList3) {
            log.debug("clean: Removing node with class: " + node.className());
            removeNode(node);
        }
        Elements naughtyList4 = children.select(queryNaughtyClasses);
        log.debug(naughtyList4.size()
        ...
        log.debug(naughtyList5.size() + " naughty Name elements found");
        for (Element node : naughtyList5) {
            log.debug("clean: Removing node with class: "
    ...
    private void removeNodesViaRegEx(Document doc, Pattern pattern) {
        try {
            Elements naughtyList = doc.getElementsByAttributeValueMatching(
                    "id", pattern);
            log.debug("regExRemoveNodes: " + naughtyList.size()
            ...
            log.debug("regExRemoveNodes: " + naughtyList3.size()
    ...
    private void removeScriptsAndStyles(Document doc) {
        log.debug("Starting to remove script tags");

        Elements scripts = doc.getElementsByTag("script");
        for (Element item : scripts) {
            item.remove();
        }
        log.debug("Removed: " + scripts.size() + " script tags");
        log.debug("Removing Style Tags");
        Elements styles = doc.getElementsByTag("style");
        for (Element style : styles) {
            style.remove();
        }
        log.debug("Removed: " + styles.size() + " style tags");
    }


    private void convertDivToParagraph(Document docToClean, String tag) {
        log.debug("Starting to replace bad divs");
    ...
    private void convertDivsToParagraphs(Document doc, String domType) {
        ...
        log.debug("Found " + divs.size() + " total divs with " + badDivs
    ...
    private Element getFlushedBuffer(StringBuilder replacementText, Document doc) {
        ...
        log.debug("Flushing TextNode Buffer: " + bufferedText.trim());
    ...
    /**
     * @param doc
     * @return Clean up span tags in paragraphs takes care of the situation
    ...
    private void cleanUpSpanTagsInParagraphs(Document doc) {
        ...
        log.debug("Cleaning " + span.size() + " span tags in paragraph ");

        for (Element item : span) {
            if (item.parent().nodeName().equals("p")) {
                TextNode tn = new TextNode(item.text(), doc.baseUri());
                item.replaceWith(tn);
                log.debug("Replacing nested span with TextNode: "
                        + item.text());
    ...
    private void cleanEmTags(Document doc) {
        ...
        log.debug("Cleaning " + ems.size() + " EM tags");
