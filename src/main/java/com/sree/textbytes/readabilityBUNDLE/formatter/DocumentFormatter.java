...
public class DocumentFormatter {
    ...
    static {
        regExRemoveNodes = "^(small|articleVersion|taglist|memo|insetFullBox|latest_blogs|list-wrapper|comments|block-ec_blogs-ec_blogs_block_bloginfo|twitter-tweet|tpcdiv)$|^ec-messages messages-warning message-region" +
                "|^(headlines assetContainer|mpsharehighlight|glInArticle|block|fourwide ie_leftborder|chrome|jitCmtySignInDialog|jitMktwSignInDialog|jitPremiumsSignInDialog)$" +
                "|^(story_social_toolbar_bottom|sixwide TopBorder|hslice|mobileapps_prom|addtoany_share_save_container)$" +
                "|^(post-meta-data|breadcrumb|nr_fo_bot_of_post|cnbc_share|ibnWdt300|PL10|PT10 b_14)$" +
                "|(comment-\\d+)";
        ...
        regExjunkNotes1 = "(?i)(Additional\\s+)?(?i)(Reporting|Writing|Polling|Editing)";
        regExjunkNotes2 = "(Write to)For more information about";
    ...
    private void removeNode(Element node) {
        if (node == null || node.parent() == null)
            return;
        log.debug("Removing Cleaning node : {}", node);
        node.remove();
    }
    ...
    private Element cleanupNode(Element node) {
        log.debug("Starting clean up Node");
        ...
        for (Element e : nodes) {
                    ...
                    if (iframeElements.size() > 0 || objectElements.size() > 0) {
                        for (Element iframe : iframeElements) {
                            if (iframe.tagName().equals("iframe")) {
                                ...
                                log.debug("IFRAME attributes : " + attributes);
                                if (Patterns.exists(Patterns.VIDEOS, attributes)) {
                                    log.debug("valid video in iframe" + iframe);
                                } else {
                                    log.debug("Removing IFRAME , not a valid video");
                                    iframe.remove();
                                }
                                ...
                                for (Element embedElement : embedElements) {
                                    ...
                                    log.debug("Object Attribute : " + srcAttribute);
                                    if (Patterns.exists(Patterns.VIDEOS, srcAttribute)) {
                                        log.debug("Valid video found in Object embed");
                                    } else {
                                        log.debug("Removing Object , not a valid video");
                                        iframe.remove();
                                    }
                                }
                            }
                        ...
                        log.debug("Clean Up text less than critical and not images" + p);
                        p.remove();
                    }
            ...
            if (e.tagName().equals("ul") || e.tagName().equals("ol")) {
                ...
                log.debug("topNodeScore: " + topNodeScore + " currentNodeScore: " + currentNodeScore + " threshold: " + thresholdScore);
                if (currentNodeScore < thresholdScore && imgElements.size() == 0 && iframeElements.size() == 0) {
                    if (!e.tagName().equals("td") && !e.tagName().equals("table") && tableElements.size() == 0) {
                        log.debug("Removing node due to low threshold score : node " + e);
                        e.remove();
                    } else {
                        log.debug("Not removing TD node");
                    }
                ...
                if (isDropCap) {
                    dropCapText = e.html();
                    log.debug("DropCap Text :" + dropCapText);
                }

                log.debug("Drop Cap is : " + isDropCap);
        ...
        if (dropCapText != null) {
            for (Element e : node.children()) {
                ...
                if (Patterns.exists(Patterns.DROP_CAP, className)) {
                    log.debug("Drop Cap Text element : " + e);
        ...
        for (Element tagElement : tagList) {
            ...
            if ((weight + contentScore) < 0) {
                ...
                if (tagElement.tagName().equals("ul") || tagElement.tagName().equals("ol")) {
                    ...
                    log.debug("Link density of ul node : " + linkDensity);
                    if (linkDensity > 0.3) {
                        log.debug("Removing list node due to high link density");
                        tagElement.remove();
                    }
                } else {
                    tagElement.remove();
                ...
                if (img > p)
                    ...
                    toRemove = false;
                if (toRemove) {
                    log.debug("Removing node toRemove is true : " + tagElement);
                    int imageSize = tagElement.select("img").size();
                    if (imageSize > 0) {
                        log.debug("Not removing since it contains some images");
                    } else {
                        tagElement.remove();
                    }
            ...
            for (Element element : elements) {
                if (element.hasText()) {
                    if (checkHtmlHashDuplicates(element.hashCode())) {
                        log.debug("Duplicate Html hash tag found in extracted output");
                        removeNode(element);
                    } else
