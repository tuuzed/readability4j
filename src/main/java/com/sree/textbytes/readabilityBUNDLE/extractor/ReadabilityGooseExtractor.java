...
public class ReadabilityGooseExtractor implements ReadabilityExtractor {
    ...
    private Element calculateBestNodeBasedOnClustering(Document doc) {
        ...
        log.debug("About to inspect num of nodes with text: " + numberOfNodes);

        for (Element node : nodesWithText) {
            log.debug("NodesWithText : " + node);
            ...
            log.debug("Location Boost Score: " + boostScore + " on iteration: " + i + "' id='" + node.parent().id() + "' class='" + node.parent().attr("class"));
            String nodeText = node.text();
            WordStats wordStats = StopWords.getStopWordCount(nodeText);
        ...
        for (Element e : parentNodes) {
            log.debug("ParentNode: score='" + e.attr("algoScore") + "' nodeCount='" + e.attr("algoNodes") + "' id='" + e.id() + "' class='" + e.attr("class") + "' " + "    :  " + e);
        ...
        if (log.isDebugEnabled()) {
            if (topNode == null) {
                log.debug("ARTICLE NOT ABLE TO BE EXTRACTED!, WE FAILED!");
            } else {
                String logText;
                String targetText = "";
                ...
                log.debug("TOPNODE TEXT: " + logText.trim());
                log.debug("Our TOPNODE: score='" + topNode.attr("algoScore") + "' nodeCount='" + topNode.attr("algoNodes") + "' id='" + topNode.id() + "' class='" + topNode.attr("class") + "' ");
            }
        }
